...
import type { PricingVariable, SupplierCandidate, SupplierSelection } from "@/types/pricing";
...
function selectedSupplierStatusIds(value: unknown): Map<number, number> {
  const selections = Array.isArray(value)
    ...
    : value && typeof value === "object"
      ? Object.entries(value as Record<string, unknown>).map(([item_id, supplier_status_id]) => ({ item_id, supplier_status_id }))
      : [];
...
export async function GET(request: Request) {
  try {
    ...
    const variables = quotation?.quotation_id
      ? await query("select * from quotation_pricing where quotation_id = $1", [quotation.quotation_id])
      : [];
    return NextResponse.json({
      ...
      items,
      calculated_pricing: variables.map((row) => ({
        item_id: numeric(row.item_id),
        sales_unit_price: numeric(row.sales_unit_price),
        ext_price: numeric(row.ext_price),
        potential_profit: numeric(row.potential_profit),
        calculation_timestamp: new Date().toISOString()
      })),
      variables: variables.map((row) => ({
        item_id: numeric(row.item_id),
        shipping_cost: row.shipping_cost == null ? null : numeric(row.shipping_cost),
        tax_rate: row.tax_rate == null ? null : numeric(row.tax_rate),
        exchange_rate: row.exchange_rate == null ? null : numeric(row.exchange_rate),
        profit_rate: row.profit_rate == null ? null : numeric(row.profit_rate),
        discount_rate: row.discount_rate == null ? null : numeric(row.discount_rate)
      }))
    });
  } catch (error) {
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Failed to load pricing." },
      { status: 500 }
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const variables = (body.variables ?? []) as PricingVariable[];
    const requestedSelections = selectedSupplierStatusIds(body.supplier_selections ?? body.selected_supplier_status_ids);
    if (!Number.isFinite(rfqId) || rfqId <= 0) {
    ...
    if (!signup.signup || !signup.company_id || !signup.user_id) {
      return NextResponse.json(
        { success: false, error: "Signup is required before pricing can be persisted." },
        { status: 400 }
      );
    }
    const companyId = signup.company_id;
      ...
      return NextResponse.json(
        {
          success: false,
          error: errors.join(". "),
          supplier_candidates: items.map((item) => ({ item_id: item.item_id, candidates: item.supplier_candidates }))
        },
        { status: 400 }
      );
    }
    ...
    if (!quotation) {
      quotation = await insertReturning("quotations", {
        ...
        transfer_currency_code: body.exchange_currency ?? "VND",
        commercial_terms: body.commercial_terms ?? "Validity, payment, warranty, and delivery terms to be confirmed."
      });
    } else {
    ...
    for (const result of calculated.calculated_pricing) {
      ...
      const payload = {
        ...
        exchange_currency: body.exchange_currency ?? "VND",
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Failed to save pricing." },
      { status: 500 }
    );
  }
}
...
async function loadPricingItems(rfqId: number): Promise<PricingItem[]> {
  ...
  return rfqItems.map((row) => {
    ...
    return {
      ...
      delivery_time: supplier?.delivery_time as string | null
    };
  });
}
