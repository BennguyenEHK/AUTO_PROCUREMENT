...
Mandatory output location: store every HTML report or preview generated by this skill inside:

```text
C:\Users\LENOVO\.codex\skills\certificate-origin-review\output
```

Before giving the user a link, verify the file exists in that folder and return both the Windows path and a Codex-openable browser link such as `file:///C:/Users/LENOVO/.codex/skills/certificate-origin-review/output/<file>.html`.

When editing this `SKILL.md` or certificate-origin Markdown/control files, preserve UTF-8 without BOM and verify no BOM after edits when practical.
...
When `bid-package-orchestrator` routes a supplier/customer reply, run this skill only when the reply affects certificates, country of origin, country of manufacture, documentation, manufacturer authorization, datasheets, drawings, bid-stage vs delivery-stage document availability, document cost inclusion/exclusion, or certificate/origin assumptions.

Do not run this skill as part of a blind three-skill bundle. For supplier replies with a revised offer, require the `supplier-quotation-normalizer` handoff first when certificate/origin statements are embedded in a changed quotation or commercial offer. This skill may run in parallel with `technical-compliance-review` only after normalized quote data exists and only when both scopes are independently affected.

For response-impact workflows, output a `Certificate / Origin / Document Impact` section for the combined Response Impact Report. It must include affected items/documents, customer requirement, supplier/OEM statement, bid-stage vs delivery-stage status, included/excluded/chargeable status, evidence source, blocker status, and recommended clarification.
...
- Certificate / Origin / Document Impact section for combined Response Impact Reports.
...
- HTML/report artifacts saved under this skill's `output` folder with verified Codex-openable `file:///...` links.
...
- Do not proceed from a supplier revised-offer reply when the quotation context has not first been normalized or blocked by `supplier-quotation-normalizer`, unless the orchestrator confirms the reply is purely a certificate/origin/document response.
